...
/**
 * Oceanpayment WeChatPay Payment Gateway
 *
 * Provides a Oceanpayment WeChatPay Payment Gateway, mainly for testing purposes.
 *
 * @class 		WC_Gateway_Oceanwechat
 * @extends		WC_Payment_Gateway
 * @version		1.4
 * @package		WooCommerce/Classes/Payment
 * @author 		Oceanpayment
 */
class WC_Gateway_Oceanwechat extends WC_Payment_Gateway {
    ...
    public function __construct() {
        $this->id                 = 'oceanwechat';
        $this->icon               = apply_filters('woocommerce_oceanwechat_icon', plugins_url( 'images/op_Wechat.png', __FILE__ ));
        $this->has_fields         = false;
        $this->method_title       = __( 'Oceanpayment WeChatPay', 'oceanpayment-wechat-gateway' );
        $this->method_description = __( '', 'oceanpayment-wechat-gateway' );
        $this->supports           = [
            'products',
//            'add_payment_method',
        ];
        ...
        $this->Title = $this->get_option('_oceanwechat_title') ? $this->get_option('_oceanwechat_title') : '';
        $this->Body = $this->get_option('_oceanwechat_description') ? $this->get_option('_oceanwechat_description') : '';

        // Actions
        add_action( 'woocommerce_api_wc_gateway_oceanwechat', array( $this, 'check_ipn_response' ) );
        add_action( 'woocommerce_update_options_payment_gateways_' . $this->id, array( $this, 'process_admin_options' ) );
        add_action( 'valid-oceanwechat-standard-itn-request', array( $this, 'successful_request' ) );
        add_action( 'woocommerce_receipt_' . $this->id, array( $this, 'receipt_page' ) );
        add_action( 'woocommerce_thankyou_oceanwechat', array( $this, 'thankyou_page' ) );
...
//        add_action( 'woocommerce_receipt_oceanwechat', array( $this, 'oceanwechat_receipt_page' ) );
    }
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type'    => 'checkbox',
                'label'   => __( 'Enable Oceanpayment WeChatPay Payment', 'woocommerce' ),
                'default' => 'yes'
            ),
            'title' => array(
                ...
                'default'     => __( 'WeChatPay Payment', 'woocommerce' ),
                'desc_tip'    => true,
            ),
            'description' => array(
	...
	public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
		if ( $this->instructions && ! $sent_to_admin && 'oceanwechat' === $order->get_payment_method() && $order->has_status( 'on-hold' ) ) {
    ...
    function receipt_page( $order ) {
        echo $this->generate_wechat_form( $order );
        die;
    }
     ...
     * 生成 WeChatPay form.
     */
    public function generate_wechat_form( $order_id ) {
        ...
        //账单人州(可不提交)
        $billing_state     = $this->get_wechat_state( $order->get_billing_country(), $order->get_billing_state() );
        //账单人城市
        ...
        //收货人的州（省、郡）
        $ship_state 	   = empty($this->get_wechat_state( $order->get_shipping_country(), $order->get_shipping_state())) ? $billing_state : $this->get_wechat_state( $order->get_shipping_country(), $order->get_shipping_state());
        //收货人的城市
        ...
        $data_to_send .= "</div>";
        $data_to_send .= "</div>";
        $data_to_send .= "<form  method='post' name='wechat_checkout' action='".$this->settings['submiturl']."'  >";
        ...
        $data_to_send .= 'document.wechat_checkout.submit();' . "\n";
        $data_to_send .= '</script>' . "\n";
            ...
            if($return_info['response_type'] == 1){
                ...
                if(strtoupper($local_signValue) == strtoupper($return_info['signValue'])){
                    ...
                    if ($return_info['payment_status'] == 1) {
                        //成功
                        $order->update_status( 'processing', __( $testorder.$return_info['payment_details'], 'oceanpayment-wechat-gateway' ) );
                        ...
                        if(empty($this->completed_orders()) || !in_array($return_info['order_number'], $this->completed_orders())){
                    		$order->update_status( 'on-hold', __( $testorder.$return_info['payment_details'], 'oceanpayment-wechat-gateway' ) );
                		}
                        ...
                        if($errorCode == '20061'){
                        	...
                        	if(empty($this->completed_orders()) || !in_array($return_info['order_number'], $this->completed_orders())){
                    			$order->update_status( 'failed', __( $testorder.$return_info['payment_details'], 'oceanpayment-wechat-gateway' ) );
                			}
                        }
    ...
    function return_payment( $order ) {
        ...
        if(strtoupper($local_signValue) == strtoupper($back_signValue)){
            ...
            if ($payment_status == 1) {
                //成功
                $order->update_status( 'processing', __( $testorder.$payment_details, 'oceanpayment-wechat-gateway' ) );
                WC()->cart->empty_cart();
                ...
                if(empty($this->completed_orders()) || !in_array($_REQUEST['order_number'], $this->completed_orders())){
                	$order->update_status( 'on-hold', __( $testorder.$payment_details, 'oceanpayment-wechat-gateway' ) );
        		}
                $url = $this->get_return_url( $order );
                wc_add_notice( $testorder.$payment_details, 'success' );
                ...
                if($errorCode == '20061'){
					...
					if(empty($this->completed_orders()) || !in_array($_REQUEST['order_number'], $this->completed_orders())){
                		$order->update_status( 'failed', __( $testorder.$payment_details, 'oceanpayment-wechat-gateway' ) );
        			}
                    $url = esc_url( wc_get_checkout_url() );
     ...
     * 获取州/省
     */
    public function get_wechat_state( $cc, $state ) {
